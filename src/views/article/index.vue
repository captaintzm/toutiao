...
<div class="article-container">
     ...
     <van-cell center class="user-info">
         ...
         round 
         @click="onFollow"
         class="follow-btn">{{article.is_followed? '已关注':'关注'}}</van-button>
     </van-cell>
      <div class="markdown-body words" ref="article-content" v-html="article.content"></div>
     <!-- 文章评论列表 -->
       <CommentList :source="articleId" :list="commentList"
        @total-count="totalCommentCount = $event" @reply-click="onReplyClick"/>
     </div>
    ...
    <van-popup 
     v-model="isPostShow"
     position="bottom">
    <PostCommment :target="articleId" @post-success = 'onPostSuccess'/>
    </van-popup>
    ...
    <van-popup 
     v-model="isReplyShow"
     position="bottom">
     <CommentReply :comment="replyComment" 
     @close="isReplyShow = false" v-if="isReplyShow"
...
import PostCommment from './components/post-comments'
...
return {
  ...
  isPostShow:false,//控制发布的显示状态
  commentList:[], //文章评论列表
  totalCommentCount: 0, // 评论总数据量
  isReplyShow:false, //控制评论的回复显示状态
...
methods: {
  async loadArticle(){
   ...
   handlePerviewImage(){
     // 1. 获取文章内容 DOM 容器,在vue中获取dom元素使用$refs
    const articleContent = this.$refs['article-content']
    // 2. 得到所有的 img 标签
    const imgs = articleContent.querySelectorAll('img')
    const imgPath = [] //收集所有的图片路径
